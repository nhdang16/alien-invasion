...
class Scoreboard():
    ...
    def __init__(self, ai_settings, screen, stats):
        ...
        self.font = pygame.font.Font(f"font/invasion.ttf", 30)
    ...
    def prep_aliens_toKill(self):
        """Turn the level into a rendered image."""
        aliens_toKill = "NEXT LEVEL: " + \
            str(self.stats.kills) + " / " + str(self.ai_settings.aliens_toKill)
        self.aliens_toKill_image = self.font.render(
            aliens_toKill, True, self.text_color, self.ai_settings.bg_color
        )

        # Position the level below the score.
        self.aliens_toKill_rect = self.aliens_toKill_image.get_rect()
        self.aliens_toKill_rect.right = self.level_rect.right
        self.aliens_toKill_rect.top = self.level_rect.bottom + 10
...
class Quit_State():
    def __init__(self, ai_settings, screen):
        ...
        self.font = pygame.font.Font(f"font/invasion.ttf", 40)

        self.quit_statement(ai_settings)

    def quit_statement(self, ai_settings):
        quit_state = "'q' to quit"
        self.font = pygame.font.Font(f"font/invasion.ttf", 20)
